...
class InputItemParser extends Parser[List[String], InputItem] {
  override def marshall(
      input: List[String]
  ): Either[DonneesIncorrectesException, InputItem] = {
    input match {
      case head :: _ =>
        if (isValidPosition(head)) {
            ...
            case _: Exception =>
              Left(
                DonneesIncorrectesException(
                  "Les instructions doivent être composées de caractères valides"
                )
              )
          }
        } else {
          Left(
            DonneesIncorrectesException(
              "La position de la tondeuse doit être composée de 2 entiers et d'une orientation séparés par un espace"
            )
          )
        }
      case _ =>
        Left(
          DonneesIncorrectesException(
            "L'input item parser doit recevoir une liste de 2 string et non :" + input.length.toString
          )
        )
    }
  }
  ...
  private def isValidPosition(position: String): Boolean = {
    ...
    values.length == 3 && values(0).forall(_.isDigit) && values(1).forall(
      _.isDigit
    ) && values(2).forall(_.isLetter)
  }
  ...
  private def parsePosition(position: String): State = {
    ...
    State(
      (positionList(0).toInt, positionList(1).toInt),
      Orientation.withName(positionList(2))
    )
  }
